...
export const useKitchen = () => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      if (response) {
        setOrders([{ data: response, status: 'pending', timestamp: new Date().toISOString() }]);
      } else {
        setOrders([]);
      }
    } catch (err) {
  ...
  const toggleMenuAvailability = useCallback(async (menuId: string, isAvailable: boolean) => {
    try {
      await kitchenApi.toggleAvailability(menuId);
      await refetchMenus();
    } catch (err) {
  ...
  const updateOrderStatus = useCallback(async (queueCode: string, newStatus: 'cooking' | 'ready') => {
    try {
      ...
      if (newStatus === 'cooking') {
        ...
        updated = await kitchenApi.finishCooking(queueCode);
      }
      if (!updated) return;
      setOrders(prev =>
        prev.map(order =>
          order.data.queue_code === queueCode ? { ...order, data: updated } : order
        )
      );
    } catch (err) {
